...
inline const char *pieceToChar(int p) {
    switch (p) {
        ...
        case EMPTY: return ".";
        default: return "?"; // for debugging weird values
    }
}
...
inline const char *pieceToSymbol(int p) {
  switch (p) {
    ...
    default: return u8"?"; // for debugging weird values
  }
}

inline const char *pieceToRepresentation(int p, bool symbol) {
  if (symbol) return pieceToSymbol(p);
...
inline void printBoard(const Position &pos, bool symbol) {
    ...
    if (pos.castlingRights == 0) std::cout << "-";
    else {
        if (pos.castlingRights & WK_CASTLE) std::cout << "K";
        if (pos.castlingRights & WQ_CASTLE) std::cout << "Q";
        if (pos.castlingRights & BK_CASTLE) std::cout << "k";
        if (pos.castlingRights & BQ_CASTLE) std::cout << "q";
    }
    std::cout << "\n";
