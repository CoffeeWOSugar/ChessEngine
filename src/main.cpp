...
bool chooseAIMove(Position &pos, Move &outMove) {
	...
	SearchLimits limits;
	limits.useTime = true;
	limits.endTime = std::chrono::steady_clock::now() + std::chrono::milliseconds(THINK_TIME_MS);
	...
	for (const Move &m : moves) {
		if (m.from == fromSq && m.to == toSq) {
			...
			if ((m.flags & MF_PROMOTION) != 0) {
				if (promoPiece == EMPTY)
					continue; // promotion required but not given
				if (m.promotion != promoPiece)
					continue;
			} else {
				if (promoPiece != EMPTY)
					continue; // user gave promotion on non-promo move
			}
			outMove = m;
			return true;
		}
...
int main(int argc, char *argv[]) {
	...
	while (true) {
		...
		if (moves.empty()) {
			...
			if (in_check) {
				std::cout << "Checkmate! "
				          << (pos.sideToMove == humanColor ? "You lose." : "You win!") << std::endl;
			} else {
