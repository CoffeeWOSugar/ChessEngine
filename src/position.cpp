...
Position::Position() { board.fill(EMPTY); }
...
bool Position::inCheck(Color c) const {
	...
	if (enemy == WHITE) {
		int sq1 = kingSq - 15;
		int sq2 = kingSq - 17;
		if (Position::isOnBoard(sq1) && board[sq1] == WP)
			return true;
		if (Position::isOnBoard(sq2) && board[sq2] == WP)
			return true;
	} else {
		int sq1 = kingSq + 17;
		int sq2 = kingSq + 15;
		if (Position::isOnBoard(sq1) && board[sq1] == BP)
			return true;
		if (Position::isOnBoard(sq2) && board[sq2] == BP)
			return true;
	}
...
bool Position::makeMove(const Move &m) {
	...
	if (capturedPiece == WR) {
		if (capturedSq == SQ_A1)
			castlingRights &= ~WQ_CASTLE;
		else if (capturedSq == SQ_H1)
			castlingRights &= ~WK_CASTLE;
	} else if (capturedPiece == BR) {
		if (capturedSq == SQ_A8)
			castlingRights &= ~BQ_CASTLE;
		else if (capturedSq == SQ_H8)
			castlingRights &= ~BK_CASTLE;
	}
...
bool Position::isSquareAttacked(int sq, Color by) const {
	...
	if (by == WHITE) {
		int sq1 = sq - 15;
		int sq2 = sq - 17;
		if (isOnBoard(sq1) && board[sq1] == WP)
			return true;
		if (isOnBoard(sq2) && board[sq2] == WP)
			return true;
	} else {
		int sq1 = sq + 15;
		int sq2 = sq + 17;
		if (isOnBoard(sq1) && board[sq1] == BP)
			return true;
		if (isOnBoard(sq2) && board[sq2] == BP)
			return true;
	}
