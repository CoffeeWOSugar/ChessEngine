...
int evaluateMaterial(const Position& pos) {
    ...
    for (int sq = 0; sq < 128; ++sq) {
        ...
        int p = pos.board[sq];
        switch (p) {
            case WP: score += PAWN_VALUE;   break;
            case WN: score += KNIGHT_VALUE; break;
            case WB: score += BISHOP_VALUE; break;
            case WR: score += ROOK_VALUE;   break;
            case WQ: score += QUEEN_VALUE;  break;
            case WK: break; // king value handled via mate scores

            case BP: score -= PAWN_VALUE;   break;
            case BN: score -= KNIGHT_VALUE; break;
            case BB: score -= BISHOP_VALUE; break;
            case BR: score -= ROOK_VALUE;   break;
            case BQ: score -= QUEEN_VALUE;  break;
            case BK: break;
        }
    }
...
bool searchBestMove(Position& pos,
                    int maxDepth,
                    const SearchLimits& limits,
                    Move& bestMove)
{
    ...
    for (int depth = 1; depth <= maxDepth; ++depth) {
        ...
        std::cout << "Depth " << depth
                  << " best score = " << currentBestScore << std::endl;
    }
